...
void Data::split_data(float test_size)
{
    ...
    // Memory only: when no test split is requested (version5 always passes test_size = 0
    // and splits indices itself) hand the loaded array over by pointer instead of copying
    // it.  The bytes are identical -- this just avoids holding two ~236 MB copies at once,
    // which was peaking at 474 MB per process and getting the sweep OOM-killed.
    if (this->train_size == this->number_of_images) {
        images_train = images;
        images_test = new Image[0];
        images = nullptr;
