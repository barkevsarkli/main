...
class Data {
    private:
        ...
        void load_images(const std::string &filename_images);
        void load_labels(const std::string &filename_labels);
...
void Data::normalize_data()
{
    ...
    for (uint32_t i = 0; i < this->number_of_images - this->train_size; i++)
        for (uint32_t j = 0; j < 28 * 28; j++)
            images_test[i].normalized_pixels[j] = images_test[i].pixels[j] / 255.0f;
